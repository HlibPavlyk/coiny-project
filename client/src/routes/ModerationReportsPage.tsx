...
import { moderation, useReports, type ReportItemModel, type ReportStatus } from '@/api/moderation';
...
export default function AdminReportsPage() {
  ...
  const dismissActions = (report: ReportItemModel): ModalAction[] => [
    { label: 'Dismiss report', onClick: () => run(() => moderation.dismissReport(report.id, note.trim() || undefined), 'Report dismissed') },
  ];
  ...
  const takeActionActions = (report: ReportItemModel): ModalAction[] => [
    {
      ...
      onClick: () =>
        run(async () => {
          await moderation.takeAction(report.id, note.trim());
          await moderation.takedownLot(report.lot.id);
    ...
    },
    {
      ...
      onClick: () =>
        run(async () => {
          ...
          await moderation.takeAction(report.id, note.trim());
          await moderation.banUser(detail.seller.id, note.trim());
