...
export class AlbumsViewComponent implements OnInit {
  ...
  getAlbums(): void {
    ...
    const searchRequest: AlbumSearchRequest = {
      page: this.page,
      size: this.size,
    ...
    if (this.isCurrentUser || this.userId) {
      ...
      this.albumService.getPagedAlbumsByUser(this.userId!, searchRequest)
          .subscribe({
            next: (response) => {
              this.totalPages = Math.ceil(response.totalCount / this.size);
      ...
      this.albumService.getPagedAlbumsForView(searchRequest)
          .subscribe({
            next: (response) => {
              this.totalPages = Math.ceil(response.totalCount / this.size);
              this.albums = response.items.map(album => {
