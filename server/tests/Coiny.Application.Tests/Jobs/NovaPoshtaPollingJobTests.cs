...
public class NovaPoshtaPollingJobTests
{
    ...
    [Fact]
    public async Task First_delivered_sets_DeliveredAt_and_schedules_capture()
    {
        using var ctx = NewDb();
        ...
        jobs.ScheduleCaptureCalls.Should().Be(1);
        jobs.LastCapturePaymentId.Should().Be(shipment.PaymentId);
        jobs.LastCaptureDelay.Should().Be(TimeSpan.FromHours(24));
    }
    ...
    {
        public int ScheduleCaptureCalls { get; private set; }
        public Guid? LastCapturePaymentId { get; private set; }
        public TimeSpan? LastCaptureDelay { get; private set; }
        ...
        {
            ScheduleCaptureCalls++;
            LastCapturePaymentId = paymentId;
            LastCaptureDelay = delay;
            return "capture-id";
        }
    }
}
