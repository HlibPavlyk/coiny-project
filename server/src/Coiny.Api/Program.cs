...
// CORS for the SPA calling the API across origins. Cross-origin requests with the HttpOnly auth
// cookie require BOTH a non-wildcard `Access-Control-Allow-Origin` AND
// `Access-Control-Allow-Credentials: true` — `WithOrigins` + `AllowCredentials` set those.
// Origins come exclusively from configuration (`Cors:AllowedOrigins`); the deploy controls them
// without code changes. Missing config = no allowed origins = every preflight fails fast and
// loudly, which is better than a silently-broken catch-all default.
...
// CORS must run BEFORE Authentication/Authorization — the preflight OPTIONS request carries no
// cookie/auth header, so if Authentication is hit first it returns 405/401 and the real request
// never gets to fire.
app.UseCors("spa");
